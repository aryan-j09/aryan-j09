...
$(document).ready(function() {
    // Initialize select2
    if(!('<?= $id ?>')) {
        $('#po_code').select2({width:'100%'}).on('change', function() {
...
function generatePaymentTerms(data) {
    ...
    const payments = [
        ...
        { type: 'inspection', label: inspectionLabel },
        { type: 'installation', label: 'Installation' },
        { type: 'credit', label: `${data.credit_payment_days || 0} Days Credit` }
    ];

    payments.forEach(({type, label}) => {
        if (type === 'credit') {
