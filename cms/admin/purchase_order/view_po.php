...
$qry = $conn->query("SELECT p.*, s.name as supplier, s.address as supplier_address, s.contact as supplier_contact, 
    ...
    WHERE p.id = '{$_GET['id']}'");
if(!$qry) { die('Query error (purchase_order_list): ' . $conn->error); }
if($qry->num_rows > 0){
...
if(!$item_query) { die('Query error (po_items): ' . $conn->error); }
$items = [];
...
<style>
    ...
    @media print {
        ...
        #authorized_signatory { display: none !important; }
        #signatory_display { display: inline !important; font-style: italic; }        
    }
    .sbp-logo {
        height: auto;
        width: auto;
    ...
    <div class="card-body" id="print_out">
        <div class="container-fluid">
            ...
            <div class="po-address-block">
                ...
                <p>Yours truly,<br>
                <label>For <?php echo ($company_type == 'HUGO') ? 'Hugopharm Technologies Pvt. Ltd.' : 'S.B. Panchal & Company'; ?></label></p>
